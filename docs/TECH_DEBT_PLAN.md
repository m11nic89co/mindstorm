...
> **Статус:** верификация финальной приёмки (ожидание CI после push).  
...
| 2. Unit-тесты `groupResize` | High | ✅ закрыт | 8 / 8 |
| 3. CI quality gate | Medium | ✅ закрыт | 5 / 5 |
| 4. Node policy | Medium | 🟡 верификация CI | 3 / 4 |
| 5. i18n / a11y хвосты | Low | ✅ закрыт | 4 / 4 |
| Финальная приёмка | — | 🟡 в процессе | 4 / 6 |

**Общий прогресс:** 30 / 33 чекпоинта (ожидание: CI на `main` после `git push`)
...
- [x] Resize группы с карточками внутри (пропорциональное масштабирование).
...
- [x] **2.2** Тест: text-карточка с центром внутри группы масштабируется пропорционально при resize root-группы.
- [x] **2.3** Тест: вложенная группа и её содержимое масштабируются рекурсивно (`nodesInsideGroupTree`).
...
- [ ] CI на чистой ветке проходит полностью.
- [ ] Искусственно сломанный тест — CI падает (проверка gate).
...
- [ ] **4.4** CI проходит без deprecation warning по Node (проверка лога Actions).
...
- [ ] **F.1** Все чекпоинты этапов 1–5 отмечены выполненными (ожидает **4.4**).
- [x] **F.2** `npm run test` и `npm run build` проходят локально.
- [ ] **F.3** CI на `main` зелёный после merge.
...
- [ ] **F.6** Таблица «Сводка прогресса» в начале этого файла — все этапы ✅.
