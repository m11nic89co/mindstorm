...
7. Сохранение: системный **Сохранить как** (`showSaveFilePicker` сразу по клику); имя = `suggestSaveTitle` (префикс прошлого + текущий timestamp); `.mindstorm` + PNG в `png/`; подсказки toolbar — снизу от курсора.
