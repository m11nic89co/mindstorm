...
**PNG + JSON:** кнопка «Сохранить» открывает **системный** диалог «Сохранить как» (имя и папка). Предлагаемое имя: префикс прошлого файла + текущие дата/время (`SUH2026-07-17_10-13-28` → `SUH` + сейчас). Пишет **`.mindstorm`** и PNG-превью в подпапку **`png/`**. PNG **не** открывается обратно в редактор.
