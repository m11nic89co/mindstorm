...
**PNG + JSON:** кнопка «Сохранить» открывает диалог **«Сохранить как»** (имя можно менять каждый раз), затем пишет **`.mindstorm`** в корень выбранной папки и PNG-превью с тем же именем в подпапку **`png/`** (создаётся автоматически). PNG **не** открывается обратно в редактор — для правок нужен `.mindstorm`.
