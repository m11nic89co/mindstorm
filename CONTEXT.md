...
│   │   ├── printLayout.ts  ← масштаб 50%, центр viewport при печати
...
- **Печать** — диалог `PrintBoardModal`: вся схема или выделенный фрагмент (`printBoard.ts`); `fitView` → масштаб **50%** (`printLayout.ts`); `@page` A4 landscape, по центру; **MiniMap/Controls не рендерятся** (`isPrinting`); history/persist на паузе; после `afterprint` — восстановление. DPI 300 — в диалоге принтера ОС.

### `Toolbar.tsx`

- **Undo/Redo** — крупные SVG-кнопки слева.
- **+ Карточка** / **T Текст** / **◻ Группа** — добавить узлы.
...
- `PRINT_SCALE = 0.5`, `viewportAtScale` — сохранить центр при уменьшении zoom.
- `PrintBoardModal` — выбор scope + подсказка layout (A4 / 50% / 300 DPI).
...
Печать: `@page { size: A4 landscape }`, центрирование shell, скрытие `.no-print` / minimap / background / controls.
...
| **Печать** (иконка 🖨) | Диалог: **Вся схема** / **Только выделенное**. Layout: A4 альбом, центр, 50%, без мини-карты; 300 DPI — в диалоге ОС. |
| **☀ / ☾** | Переключение светлой/тёмной темы (`mindstorm.theme.v1`). |
| **T Текст** | Простой текст на холсте (цвет + размер шрифта). |
...
| 2026-07 | Печать: A4 landscape, центр, масштаб 50% (`printLayout.ts`); кнопка-принтер у темы; MiniMap/Controls снимаются с DOM на время печати (`isPrinting`) |
