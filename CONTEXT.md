...
│   │   ├── Toolbar.tsx     ← toolbar, тема, печать, RU/EN/ES/中
...
│   │   ├── cardTypography.ts, groupLabel.ts ← размеры шрифтов
│   │   ├── localBoardFile.ts
│   │   ├── boardStorage.ts
...
- **Resize группы** — меняется рамка; **выделенные** text-карточки и группы масштабируются; невыделенные не трогаются. Закреплённую группу нельзя resize.
- **Сначала** — подтверждение → пустая схема; **история не сбрасывается** → можно **Undo**.
- **↺ Демо** — `getDemoCanvas(locale)` с анимацией появления.
- **Печать** — диалог `PrintBoardModal`: вся схема или выделенный фрагмент (`printBoard.ts`); на время печати скрываются лишние узлы (`hidden`), history/persist на паузе (`dragPausedRef`); после `afterprint` — восстановление state и viewport.

### `Toolbar.tsx`

- **Undo/Redo** — крупные SVG-кнопки слева.
- **Печать** — открывает диалог (вся схема / только выделенное).
- **Тема** — кнопка ☀/☾ (`useTheme().toggleTheme`).
- **Сначала** — акцентная кнопка (бирюзовая); **↺ Демо** — обычная.
- **RU | EN | ES | 中** — переключатель языка (сохраняется в localStorage).
- **SelectionPanel** — карточка: заголовок, **размер заголовка**, **размер текста**, 12 цветов; группа: название, **размер метки** (до 200 px), 12 цветов.
- **EdgeSelectionPanel** — подпись связи, удаление, перенаправление.
- Chrome UI опирается на CSS-переменные `--ms-*` (см. `index.css`).

### `theme/`

- `ThemeProvider` — контекст `{ theme, setTheme, toggleTheme }`; `document.documentElement.dataset.theme`.
- `themeStorage.ts` — ключ `mindstorm.theme.v1` (`light` | `dark`, по умолчанию `dark`).
- Стили: переменные `--ms-*` в `index.css`; карточки — `resolveColor(color, theme)`.
...
- `PrintBoardModal` в `FileModals.tsx` — выбор scope (`all` | `selection`).
...
- Расширение MindStorm: у text-узлов поле `label` — **заголовок** внутри карточки (верх); `text` — **тело** (низ). Поля независимы.
...
- **Ctrl+C** — snapshot выделенных `textCard` / `groupCard`.
...
Печать: `@media print` скрывает `.no-print` (toolbar, footer, controls, модалки).
...
| **Печать** | Диалог: **Вся схема** или **Только выделенное** (клик / Shift / ПКМ-рамка). |
