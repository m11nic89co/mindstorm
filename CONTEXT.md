...
│   │   │   └── edgeHandles.tsx
│   │   ├── edges/
│   │   │   └── MindSmoothStepEdge.tsx ← HTML-подписи связей (печать без обрезки)
│   ├── context/
│   │   └── canvasActions.ts
│   ├── hooks/
│   │   ├── useCanvasHistory.ts
...
- Print CSS: подписи рёбер — `.ms-edge-label` (серый `#94a3b8`), `white-space: nowrap`, без max-width/ellipsis.
- `MindSmoothStepEdge` — `EdgeLabelRenderer` вместо SVG `EdgeText` (getBBox на печати обрезал длинные подписи).
...
- **16 точек связи** (по 4 на сторону: `a`–`d` на 12.5% / 37.5% / 62.5% / 87.5%), `ConnectionMode.Loose` — только у карточек и групп.
...
### `flowEdges.ts` / `MindSmoothStepEdge.tsx` / `edgeLabel.ts`
...
- Подпись связи — HTML через `EdgeLabelRenderer` (`.ms-edge-label`), не SVG `EdgeText` (иначе длинный текст режется на печати).
- Панель связи: текст, **`labelFontSize`** (8–36), **`labelColor`** (палитра 1–12) — в файле как MindStorm-поля ребра.
- **Авто-разведение коридоров** (`assignEdgeLaneOffsets`): несколько связей с одной стороны узла получают разный `offset` + `stepPosition` (runtime, в файл не пишется). `MindSmoothStepEdge` обязан передавать `pathOptions` в `getSmoothStepPath`.
...
Печать: `@page { size: A4 landscape }`, тёмный текст карточек, HTML-подписи рёбер `.ms-edge-label`, скрытие `.no-print` / minimap / background / controls.
...
| 2026-07 | Save: PNG-превью в подпапку **`png/`** (создаётся автоматически) |
| 2026-07 | **Сохранить** = **Сохранить как** (диалог имени при каждом нажатии) |
| 2026-07 | Save: системный диалог ОС; имя = префикс прошлого + текущий timestamp (`suggestSaveTitle`) |
| 2026-07 | Подписи связей — HTML `MindSmoothStepEdge` (полный текст на печати) |
| 2026-07 | Подписи связей: **размер и цвет** (`labelFontSize` / `labelColor` в панели и файле) |
| 2026-07 | **16 handles** (по 4 на сторону); авто-разведение коридоров (`offset` + `stepPosition` в `flowEdges.ts`) |
