...
- **Группы:** сплошная рамка, метка на верхней кромке с **замком** справа; `NodeResizer` → resize; **замок закрыт** (`locked`) → группа фон, активен только замок.
...
- Группа: `labelFontSize` метки (8–**200** px, default 12); badge масштабируется (`groupLabelBadgeStyle`).
