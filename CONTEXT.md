...
- **Resize группы** — меняется рамка; **выделенные** text-карточки и группы с центром внутри bbox масштабируются пропорционально; невыделенные не трогаются (`groupResize.ts`, `NodeResizer` в `GroupCardNode`).
...
- **SelectionPanel** — заголовок карточки / название группы + 12 цветов (6×2).
...
- **Группы:** сплошная рамка, метка на верхней кромке; `NodeResizer` → `onGroupResizeStart/Resize/End`.
...
| **Клик по узлу** | Панель: заголовок (карточка) / название (группа) + цвет. |
