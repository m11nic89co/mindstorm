...
- Save: в выбранную **папку saves** — `.mindstorm` (JSON) в корень и PNG-превью в подпапку **`png/`** (создаётся, если нет).
...
### `flowEdges.ts` / `MindSmoothStepEdge.tsx`
...
| **Сохранить** (💾) | Диалог **«Сохранить как»**: имя файла можно менять каждый раз. `.mindstorm` в корень + **PNG** в `png/`. Папка запоминается. Toast. |
