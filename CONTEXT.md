...
- Справа: **📂 Загрузить → 💾 Сохранить → 🖨 Печать → ☀/☾ Тема** — `IconToolbarButton` одного размера.
...
| Действие | Поведение |
|----------|-----------|
| **Загрузить** (📂) | Справа в группе иконок. `.mindstorm` / `.canvas` из той же папки (если handle сохранён). PNG не открывается. Toast «Открыто: …». |
| **Сохранить** (💾) | Рядом: 📂 → 💾 → 🖨 → тема. По умолчанию **PNG**; в picker также `.mindstorm`. Chrome/Edge: «Сохранить как» + запоминание папки; иначе «Загрузки». Toast. |
| **Печать** (🖨) | Диалог: **Вся схема** / **Только выделенное**. Layout: A4 альбом, весь холст, читаемый текст, серые подписи связей, без мини-карты; 300 DPI — в диалоге ОС. |
| **☀ / ☾** | Переключение светлой/тёмной темы (`mindstorm.theme.v1`). |
| **T Текст** → **Карточка** → **Группа** | Порядок кнопок слева направо (после Undo/Redo). |
...
| 2026-07 | **📂→💾→🖨→тема** справа в toolbar; на печати подписи связей — серый фон |
