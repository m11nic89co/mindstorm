...
- **Печать** — диалог `PrintBoardModal`: вся схема или выделенный фрагмент (`printBoard.ts`); `setPrintLight(true)` + `fitView` → `PRINT_SCALE` **1** (`printLayout.ts`); `@page` A4 landscape; **MiniMap/Controls не рендерятся** (`isPrinting`); history/persist на паузе; после `afterprint` — восстановление. DPI 300 — в диалоге принтера ОС. Print CSS форсирует тёмный текст карточек (иначе white-on-white в dark).

### `Toolbar.tsx`

- **Undo/Redo** — крупные SVG-кнопки слева.
- **T Текст** → **+ Карточка** → **◻ Группа** — добавить узлы (слева направо).
- **🖨 Печать** и **☀/☾ Тема** — иконки одного размера рядом (справа от основных кнопок).
- **Сохранить** — PNG по умолчанию; **Загрузить** — `.mindstorm` / `.canvas` (папка из IndexedDB).
...
- Save: **PNG** по умолчанию (превью); опционально `.mindstorm` / legacy `.mindshtorm`; open — `.mindstorm` / `.canvas`.
- Папка save/open: File System Access `startIn` + IndexedDB `mindstorm.fs.v1` (`fileHandleStorage.ts`).
...
Печать: `@page { size: A4 landscape }`, тёмный текст карточек, скрытие `.no-print` / minimap / background / controls.
...
| Действие | Поведение |
|----------|-----------|
| **Сохранить** | По умолчанию **PNG**; в picker также `.mindstorm`. Chrome/Edge: «Сохранить как» + запоминание папки; иначе «Загрузки». Toast. |
| **Загрузить** | `.mindstorm` / `.canvas` из той же папки (если handle сохранён). PNG не открывается. Toast «Открыто: …». |
| **Печать** (иконка 🖨) | Диалог: **Вся схема** / **Только выделенное**. Layout: A4 альбом, весь холст, читаемый текст, без мини-карты; 300 DPI — в диалоге ОС. |
| **☀ / ☾** | Переключение светлой/тёмной темы (`mindstorm.theme.v1`). |
| **T Текст** → **Карточка** → **Группа** | Порядок кнопок слева направо. |
...
| 2026-07 | Save: PNG по умолчанию + запоминание папки (`exportPng`, `fileHandleStorage`); toolbar **Текст → Карточка → Группа** |
