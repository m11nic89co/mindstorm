...
- Справа: **Сначала (лист+) → 📂 Загрузить → 💾 Сохранить → 🖨 Печать → ☀/☾ Тема** — `IconToolbarButton` одного размера; «Сначала» с accent.
...
| **Загрузить** (📂) | Рядом: Сначала → 📂 → 💾 → 🖨 → тема. `.mindstorm` / `.canvas`. PNG не открывается. |
| **Сохранить** (💾) | По умолчанию **PNG**; в picker также `.mindstorm`. Запоминание папки. Toast. |
...
| 2026-07 | Иконки справа: **Сначала → 📂 → 💾 → 🖨 → тема**; «Сначала» — лист с +; **Демо** первая перед Текст |
