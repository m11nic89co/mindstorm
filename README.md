...
- Текстовые **карточки** (заголовок + текст) и **группы** с изменением размера
- **Resize группы** — меняется рамка; **выделенные** карточки и вложенные группы внутри (по центру в рамке) масштабируются пропорционально; невыделенные остаются без изменений
...
3. **Двойной клик** по **верху** карточки — заголовок; по **низу** — текст (редактируются отдельно)
4. **Точки на карточке** (при наведении) — потяните для связи
5. **Клик по карточке или группе** — заголовок и цвет справа
6. **Угол группы** — растянуть рамку; **выделенные** карточки внутри масштабируются вместе с ней (Shift+клик для множественного выделения)
7. **Сохранить** / **Загрузить** — файл на компьютер
8. **Сначала** — новая пустая схема; **↺ Демо** — пример
9. **RU | EN | ES | 中** — язык интерфейса
10. **Delete** — удалить выделенное
11. **Колёсико** — zoom (сильное отдаление для больших схем)
