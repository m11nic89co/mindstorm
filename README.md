...
5. **Клик по карточке или группе** — название, размер шрифта и цвет справа
6. **Угол группы** — растянуть рамку; **выделенные** карточки внутри масштабируются (Shift+клик)
7. **Замок** на метке группы (справа) — закрепить как фон или снова разрешить редактирование
8. **Ctrl+C / Ctrl+V** — копия выделенной карточки или группы
9. **Сохранить** / **Загрузить** — файл на компьютер
10. **Печать** — выбрать «Вся схема» или «Только выделенное» (сначала выделите узлы)
11. **☀ / ☾** — светлая или тёмная тема
12. **Сначала** — новая пустая схема; **↺ Демо** — пример
13. **RU | EN | ES | 中** — язык интерфейса
14. **Delete** — удалить выделенное
15. **Колёсико** — zoom (сильное отдаление для больших схем)
